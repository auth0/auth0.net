...
public class UserId
{
    private UserId(string type, object? value)
    {
        Type = type;
        Value = value;
    }
    ...
    /// </summary>
    public static UserId FromString(string value) => new("string", value);

    /// <summary>
    /// Factory method to create a union from a int value.
    /// </summary>
    public static UserId FromInt(int value) => new("int", value);
    ...
    public static implicit operator UserId(string value) => new("string", value);

    public static implicit operator UserId(int value) => new("int", value);

    [Serializable]
    internal sealed class JsonConverter : JsonConverter<UserId>
    {
        ...
        )
        {
            ...
            if (reader.TokenType == JsonTokenType.Number)
            {
                ...
                {
                    UserId intResult = new("int", intValue);
                    return intResult;
                }
            }
            ...
            if (reader.TokenType == JsonTokenType.String)
            {
                ...
                {
                    UserId intFromStringResult = new("int", intFromStringValue);
                    return intFromStringResult;
                }

                UserId stringResult = new("string", stringValue);
                return stringResult;
            }

            throw new JsonException(
                $"Cannot deserialize JSON token {reader.TokenType} into UserId"
            );
        }
        ...
        {
            var stringValue = reader.GetString()!;
            UserId result = new("string", stringValue);
            return result;
        }
