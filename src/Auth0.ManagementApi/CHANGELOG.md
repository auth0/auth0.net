...
_`OrganizationTemplates` sub-client removed - [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))_
- The top-level `client.OrganizationTemplates` sub-client introduced in mgmt-10.1.0 has been removed along with its associated request and parameter types (`CreateOrganizationTemplateRequestContent`, `UpdateOrganizationTemplateRequestContent`, `ListOrganizationTemplatesRequestParameters`, `ListTemplateOrganizationsRequestParameters`). Any code that references `client.OrganizationTemplates` will fail to compile and must be removed.

**Added**
- **Guardian tenant-level MFA settings:** `client.Guardian` now exposes `GetAsync()` and `SetAsync()` to read and write tenant-level MFA settings - whether the "Remember Me" checkbox is shown on the Universal Login MFA prompt and its default checked state, plus the inactivity and overall session timeouts that control how long an MFA session remains valid [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Guardian email and phone factor settings:** Two new sub-clients, `client.Guardian.Factors.Email` and `client.Guardian.Factors.Phone`, each expose `GetAsync()` and `SetAsync()` to configure factor-specific settings such as OTP length and expiry for the email and phone MFA factors [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Anonymous sessions on application clients:** Application clients now support an `AnonymousSessions` property on create, update, and read operations. Setting `Active = true` permits the client to initiate anonymous sessions [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Anonymous session tenant settings:** Tenant settings now expose a `Sessions.Anonymous` block (`TenantSettingsSessionsAnonymous`) for configuring anonymous sessions at the tenant level - the session lifetime in minutes (defaulting to 30 days, up to 1 year) and whether the `auth0_anon` cookie is issued [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Anonymous user access policy on resource servers:** Resource server subject-type authorization now includes a `ResourceServerSubjectTypeAuthorizationAnonymousUser` block, with a `Policy` field that controls whether anonymous users may obtain access tokens for the API - either `deny_all` or `require_client_grant` [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Organization search API:** `client.Organizations.SearchAsync(SearchOrganizationsRequestParameters)` enables Lucene and SCIM-syntax queries over organizations, returning cursor-paginated results (`Pager<SearchOrganization>`). Requests support a free-text query (`Q`), parser selection (`SearchParserEnum.Lucene` / `Scim`), sort by `OrganizationSortFieldEnum`, and checkpoint pagination via `From` / `Take` [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Resource server search API:** `client.ResourceServers.SearchAsync(SearchResourceServersRequestParameters)` adds the same query-and-pagination capability for APIs/resource servers, returning `Pager<ResourceServerSearchResponse>` and supporting Lucene or SCIM queries, field projection (`Fields`, `IncludeFields`), and sort by `ResourceServerSortFieldEnum` [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Experimentation: advance ramp:** A new `client.Experimentation.Experiments.AdvanceRampAsync(id, request)` call progresses a feature-flag experiment's traffic ramp. `client.Experimentation` is now part of the `IManagementApiClient` interface, and a new `OauthScope.UpdateExperimentation` (`update:experimentation`) scope is available for authorizing these operations [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Gateway timeout error:** A typed `GatewayTimeoutError` is now raised on HTTP 504 responses, making upstream-timeout failures distinguishable from other error conditions [\#1108](https://github.com/auth0/auth0.net/pull/1108) ([fern-api[bot]](https://github.com/apps/fern-api))
...
**Added**
- **Organization-level roles:** Roles can now be scoped to an organization instead of the tenant. Role create requests and responses carry a role `type` (`tenant` or `organization`, defaulting to `tenant`) and an `owner_id` identifying the owning organization, and role listing can be filtered by both. A new `client.Organizations.Roles.Groups.ListAsync(...)` sub-client lists the groups assigned to a role within an organization using checkpoint pagination, and four `*:organization_clients` OAuth scopes were added. Note that the existing role-member and role-user listings return **direct assignments only** - group-based assignments are retrieved through the new groups listing or the organization effective-roles endpoint [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Organization-to-app entitlement:** A new `client.Organizations.Clients` sub-client associates clients with an organization and manages those associations - list (checkpoint paginated), associate one or more clients, remove associations, and get or update a single association, including whether a client is used for member access. Organizations expose an `is_app_entitlement_active` flag on create, update, and read, and organization listing accepts an `include_client_association_for` client ID that annotates each returned organization with the matching client association [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Google Workspace group synchronization:** Directory provisioning gains incremental group-selection management - `AddSynchronizedGroupSelectionsAsync(...)` and `DeleteSynchronizedGroupSelectionsAsync(...)` add and remove individual groups without replacing the whole selection set, complementing the existing full-replacement call. Synchronized groups now report their name, email, and direct member count, group listing supports a Lucene prefix-search query, and Self-Service SSO tickets can enable group sync for Google Workspace alongside user sync [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Agents client:** New top-level `client.Agents` sub-client for managing agent identities - list (checkpoint paginated), create, read, update, and delete. Agents carry a name, arbitrary metadata, an optional associated client ID, and an optional immutable tenant-unique external identifier. Four `*:agents` OAuth scopes were added for these operations [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Guardian factor settings:** MFA factors can now return factor-specific settings (OTP length and OTP expiration time) when requested [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Connected accounts without an upstream user ID:** Connection connected-accounts configuration accepts `allow_missing_user_id`, permitting at most one connected account per user per connection with no upstream identity provider user ID. The default preserves the existing strict behaviour [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))
- **Event stream delivery timing:** Delivery attempts now report their `duration` in milliseconds, and the `country_codes` ACUL screen context was added [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))
- **`include_totals` on more listings:** Client grants, client-grant organizations, connections, flow executions, groups, organizations, and organization members listings now accept `include_totals` to return a result count alongside the results [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))

**Changed**
- Enabling an organization connection can now surface a `409 Conflict`, with `ConflictSchema` and `NotFoundSchema` added as typed error bodies [\#1088](https://github.com/auth0/auth0.net/pull/1088) ([fern-api[bot]](https://github.com/apps/fern-api))
...
## [mgmt-9.1.0](https://github.com/auth0/auth0.net/tree/mgmt-9.1.0) (2026-07-22)
[Full Changelog](https://github.com/auth0/auth0.net/compare/mgmt-9.0.0...mgmt-9.1.0)

**Added**
- Cross-App Access (resource-app side): Added `CrossAppAccessResourceApp` support to connections via `CreateCrossAppAccessResourceApp` / `UpdateCrossAppAccessResourceApp` request types, the `ConnectionCrossAppAccessResourceApp` response type, and the `CrossAppAccessResourceAppStatusEnum` / `ConnectionCrossAppAccessResourceAppStatusEnum` status enums. A `CrossAppAccessResourceApp` property is added to connection create/update requests and responses (`CreateConnectionRequestContent`, `UpdateConnectionRequestContent`, `CreateConnectionRequestContentOidc`, `UpdateConnectionRequestContentOidc`, `ConnectionResponseContentOidc`, `ConnectionForList`, and the connection response types), letting a connection act as the resource-app side of a Cross App Access relationship [\#1072](https://github.com/auth0/auth0.net/pull/1072) ([fern-api[bot]](https://github.com/apps/fern-api))
- Identity Assertion Authorization Grant (ID-JAG for Cross App Access): Added `IdentityAssertionAuthorizationGrant` type with `CreateIdentityAssertionAuthorizationGrant` / `UpdateIdentityAssertionAuthorizationGrant` request types, exposed as an `IdentityAssertionAuthorizationGrant` property on `CreateClientRequestContent`, `UpdateClientRequestContent`, and the `Client` response. When `active`, a client can exchange ID-JAGs for access tokens [\#1072](https://github.com/auth0/auth0.net/pull/1072) ([fern-api[bot]](https://github.com/apps/fern-api))
- Session Actor: Added `SessionActorMetadata` type and the `SessionActorClaimValue` union (string/bool/number), exposed as an `Actor` property on the session response types (`GetSessionResponseContent`, `SessionResponseContent`, `UpdateSessionResponseContent`) - represents the delegating party on delegated sessions (`sub` plus up to 5 additional primitive claims) [\#1072](https://github.com/auth0/auth0.net/pull/1072) ([fern-api[bot]](https://github.com/apps/fern-api))
- Self-Service Profiles: Added `ThirdPartyClientAccessConfig` type and a `ThirdPartyClientAccessConfig` property on `CreateSelfServiceProfileSsoTicketRequestContent`, controlling whether third-party apps can configure the connection as a domain-level connection during Self-Service Enterprise Configuration [\#1072](https://github.com/auth0/auth0.net/pull/1072) ([fern-api[bot]](https://github.com/apps/fern-api))
- Branding Themes: Added `BrandingThemeIdentifiers` type with login display, OTP autocomplete, and phone display formatting/masking enums (`BrandingThemeIdentifiersLoginDisplayEnum`, `BrandingThemeIdentifiersPhoneDisplay`, `BrandingThemeIdentifiersPhoneDisplayFormattingEnum`, `BrandingThemeIdentifiersPhoneDisplayMaskingEnum`), plus an `Identifiers` property on the theme create/update request and response types (`CreateBrandingThemeRequestContent`, `UpdateBrandingThemeRequestContent`, `CreateBrandingThemeResponseContent`, `UpdateBrandingThemeResponseContent`, `GetBrandingThemeResponseContent`, `GetBrandingDefaultThemeResponseContent`) [\#1072](https://github.com/auth0/auth0.net/pull/1072) ([fern-api[bot]](https://github.com/apps/fern-api))
- Attack Protection: Added `SuspiciousIpThrottlingPreCustomTokenExchangeStage`, added as a `pre-custom-token-exchange` stage on `SuspiciousIpThrottlingStage` [\#1072](https://github.com/auth0/auth0.net/pull/1072) ([fern-api[bot]](https://github.com/apps/fern-api))
- `TenantDomain` client option: Added `ClientOptions.TenantDomain`; when set without an explicit `BaseUrl`, `ManagementApiClient` derives `BaseUrl` as `https://{TenantDomain}/api/v2`. An explicit `BaseUrl` always takes precedence [\#1072](https://github.com/auth0/auth0.net/pull/1072) ([fern-api[bot]](https://github.com/apps/fern-api))
- Response decompression: Added gzip/deflate response decompression via a new `DefaultHttpClientFactory` [\#1072](https://github.com/auth0/auth0.net/pull/1072) ([fern-api[bot]](https://github.com/apps/fern-api))
...
**Fixed**
- mgmt: Fixes an issue in parsing JobErrorDetails when the job returns an empty array [\#921](https://github.com/auth0/auth0.net/pull/921) ([kailash-b](https://github.com/kailash-b))
