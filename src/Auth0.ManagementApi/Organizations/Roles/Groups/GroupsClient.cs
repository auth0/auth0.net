...
public partial class GroupsClient : IGroupsClient
{
    ...
    /// </summary>
    private WithRawResponseTask<ListOrganizationRoleGroupsResponseContent> ListInternalAsync(
    ...
    {
        return new WithRawResponseTask<ListOrganizationRoleGroupsResponseContent>(
            ListInternalAsyncCore(organizationId, roleId, request, options, cancellationToken)
        );
    }

    private async Task<
        WithRawResponse<ListOrganizationRoleGroupsResponseContent>
    ...
    )
    {
        ...
        var response = await _client
            .SendRequestAsync(
                new JsonRequest
                {
                    ...
                    Path = string.Format(
                        "organizations/{0}/roles/{1}/groups",
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<ListOrganizationRoleGroupsResponseContent>(
                    responseBody
                )!;
                return new WithRawResponse<ListOrganizationRoleGroupsResponseContent>()
                {
                    Data = responseData,
    ...
    /// );
    /// </code></example>
    public async Task<Pager<RoleGroup>> ListAsync(
        string organizationId,
        string roleId,
        ListOrganizationRoleGroupsRequestParameters request,
        RequestOptions? options = null,
