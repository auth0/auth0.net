...
public partial class OrganizationsClient : IOrganizationsClient
{
    ...
    /// </summary>
    private WithRawResponseTask<SearchOrganizationsPaginatedResponseContent> SearchInternalAsync(
        SearchOrganizationsRequestParameters request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<SearchOrganizationsPaginatedResponseContent>(
            SearchInternalAsyncCore(request, options, cancellationToken)
        );
    }

    private async Task<
        WithRawResponse<SearchOrganizationsPaginatedResponseContent>
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            {
                var responseData =
                    JsonUtils.Deserialize<SearchOrganizationsPaginatedResponseContent>(
                        responseBody
                    )!;
                return new WithRawResponse<SearchOrganizationsPaginatedResponseContent>()
                {
                    Data = responseData,
                    RawResponse = new Auth0.ManagementApi.RawResponse()
                    {
    ...
    /// );
    /// </code></example>
    public async Task<Pager<SearchOrganization>> SearchAsync(
        SearchOrganizationsRequestParameters request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
