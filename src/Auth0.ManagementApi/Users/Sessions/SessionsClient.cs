...
public partial class SessionsClient : ISessionsClient
{
    ...
    /// </summary>
    private WithRawResponseTask<ListUserSessionsPaginatedResponseContent> ListInternalAsync(
        string userId,
    ...
    {
        return new WithRawResponseTask<ListUserSessionsPaginatedResponseContent>(
            ListInternalAsyncCore(userId, request, options, cancellationToken)
        );
    }

    private async Task<
        WithRawResponse<ListUserSessionsPaginatedResponseContent>
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                ...
                return new WithRawResponse<ListUserSessionsPaginatedResponseContent>()
                {
                    Data = responseData,
    ...
    /// );
    /// </code></example>
    public async Task<Pager<SessionResponseContent>> ListAsync(
        string userId,
        ListUserSessionsRequestParameters request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        >
            .CreateInstanceAsync(
                ...
                response => response.Sessions?.ToList(),
                cancellationToken
            )
            .ConfigureAwait(false);
        return pager;
    ...
    {
        return new WithRawResponseTask(DeleteAsyncCore(userId, options, cancellationToken));
    }
}
