...
namespace Auth0.ManagementApi
{
    ...
    public class HttpClientManagementConnection : IManagementConnection, IDisposable
    {
        ...
        {
            return await Retry(async () => await GetAsyncInternal<T>(uri, headers, converters, token)).ConfigureAwait(false);
        }
        ...
        {
            return await Retry(async () => await SendAsyncInternal<T>(method, uri, body, headers, files, token)).ConfigureAwait(false);
        }

        private async Task<T> GetAsyncInternal<T>(Uri uri, IDictionary<string, string> headers, JsonConverter[] converters = null, CancellationToken token = default)
        {
        ...
        private async Task<T> SendAsyncInternal<T>(HttpMethod method, Uri uri, object body, IDictionary<string, string> headers, IList<FileUploadParameter> files = null, CancellationToken token = default)
        {
