...
namespace Auth0.ManagementApi.Clients
{
    ...
    public class EmailTemplatesClient : BaseClient
    {
        ...
        public Task<EmailTemplate> CreateAsync(EmailTemplateCreateRequest request, CancellationToken token = default)
        {
            return Connection.SendAsync<EmailTemplate>(HttpMethod.Post, BuildUri("email-templates"), request, DefaultHeaders, token: token);
        }
        ...
        public Task<EmailTemplate> GetAsync(EmailTemplateName templateName, CancellationToken token = default)
        {
            return Connection.GetAsync<EmailTemplate>(BuildUri($"email-templates/{templateName.ToEnumString()}"), DefaultHeaders, token: token);
        }
        ...
        public Task<EmailTemplate> PatchAsync(EmailTemplateName templateName, EmailTemplatePatchRequest request, CancellationToken token = default)
        {
            return Connection.SendAsync<EmailTemplate>(new HttpMethod("PATCH"), BuildUri($"email-templates/{templateName.ToEnumString()}"), request, DefaultHeaders, token: token);
        }
        ...
        /// <param name="token"></param>
        /// <returns>The newly updated <see cref="EmailTemplate"/>.</returns>
        public Task<EmailTemplate> UpdateAsync(EmailTemplateName templateName, EmailTemplateUpdateRequest request, CancellationToken token = default)
        {
            return Connection.SendAsync<EmailTemplate>(HttpMethod.Put, BuildUri($"email-templates/{templateName.ToEnumString()}"), request, DefaultHeaders, token: token);
        }
    }
}