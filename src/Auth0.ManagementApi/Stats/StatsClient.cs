...
public partial class StatsClient : IStatsClient
{
    ...
    private async Task<WithRawResponse<double>> GetActiveUsersCountAsyncCore(
    ...
    private async Task<WithRawResponse<IEnumerable<DailyStats>>> GetDailyAsyncCore(
    ...
    )
    {
        var _queryString = new Auth0.ManagementApi.Core.QueryStringBuilder.Builder(capacity: 2)
            ...
            .Add("to", request.To.IsDefined ? request.To.Value : null)
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                ...
                return new WithRawResponse<IEnumerable<DailyStats>>()
                {
                    Data = responseData,
                    RawResponse = new RawResponse()
                    {
    ...
    /// </code></example>
    public WithRawResponseTask<double> GetActiveUsersCountAsync(
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<double>(
            GetActiveUsersCountAsyncCore(options, cancellationToken)
        );
    }
    ...
    /// </code></example>
    public WithRawResponseTask<IEnumerable<DailyStats>> GetDailyAsync(
        GetDailyStatsRequestParameters request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<IEnumerable<DailyStats>>(
